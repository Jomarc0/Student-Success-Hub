...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT token_timestamp FROM student_credentials WHERE reset_token = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        if ($time_difference > 300) {
            ...
            $stmt = $conn->prepare("UPDATE student_credentials SET reset_token = NULL, token_timestamp = NULL WHERE reset_token = ?");
            $stmt->bind_param("s", $token);
            $stmt->execute();
...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT student_email FROM student_credentials WHERE reset_token = ?");
    $stmt->bind_param("s", $token); 
    $stmt->execute();
    $result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        if (strlen($new_password) < 8 || strlen($new_password) > 255) {
            ...
            $updateStmt = $conn->prepare("UPDATE student_credentials SET student_password = ?, reset_token = NULL WHERE student_email = ?");
